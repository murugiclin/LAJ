...
# The block reward of coinbaseoutput.nValue (500) LAJ/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 500 LAJ/block.
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.is_wallet_compiled():
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (LAJ/kB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (LAJ/kB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (500 LAJ) to the default wallet.
