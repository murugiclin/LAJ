...
class BackwardsCompatibilityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            ...
            assert_raises_rpc_error(-4, "Wallet requires newer version of Lajcoin Core", node_v19.loadwallet, "w1")
            assert_raises_rpc_error(-4, "Wallet requires newer version of Lajcoin Core", node_v19.loadwallet, "w2")
            assert_raises_rpc_error(-4, "Wallet requires newer version of Lajcoin Core", node_v19.loadwallet, "w3")
        ...
        self.stop_node(5)
        # it expected to fail with error 'DBErrors::TOO_NEW' but Lajcoin Core can open v18 by version 17
        # can be implemented in future if there's any incompatible versions
        #node_v17.assert_start_raises_init_error(["-wallet=w3_v18"], "Error: Error loading w3_v18: Wallet requires newer version of Lajcoin Core")
        #node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of Lajcoin Core")
        self.start_node(5)
