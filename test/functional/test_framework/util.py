...
def assert_fee_amount(fee, tx_size, feerate_LAJ_kvB):
    """Assert the fee is in range."""
    feerate_LAJ_vB = feerate_LAJ_kvB / 1000
    target_fee = satoshi_round(tx_size * feerate_LAJ_vB)
    if fee < target_fee:
        raise AssertionError("Fee of %s LAJ too low! (Should be %s LAJ)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * feerate_LAJ_vB:
        raise AssertionError("Fee of %s LAJ too high! (Should be %s LAJ)" % (str(fee), str(target_fee)))
