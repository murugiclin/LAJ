...
const std::array<std::string, 33> V2_BITCOIN_IDS = {
    ...
    "", /* FEEFILTER is not implemented in Lajcoin */
...
/** List of short messages allocated in Lajcoin's reserved namespace, in order.
 *
 ...
 */
const std::array<std::string, 40> V2_LAJ_IDS = {
    NetMsgType::SPORK,
 ...
 *
 * Bitcoin takes up short IDs upto 128 (lower half) while Lajcoin can take
...
constexpr std::array<std::string_view, 256> V2ShortIDs() {
    ...
    static_assert(std::size(V2_LAJ_IDS) <= 128);
    ...
    for (size_t idx{0}; idx < std::size(ret); idx++) {
        ...
        } else if (idx >= 128 && idx - 128 < std::size(V2_LAJ_IDS)) {
            ret[idx] = V2_LAJ_IDS[idx - 128];
        } else {
            ret[idx] = "";
        }
    }
...
bool IsValidV2ShortID(uint8_t first_byte) {
    ...
    // and Lajcoin's *used* slots. We do this by checking if the byte is within
    // the range where a valid message is expected to reside.
    return first_byte < std::size(V2_BITCOIN_IDS) ||
           (first_byte >= 128 && static_cast<uint8_t>(first_byte - 128) < std::size(V2_LAJ_IDS));
}
