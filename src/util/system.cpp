...
{
    // Windows: C:\Users\Username\AppData\Roaming\LajcoinCore
    // macOS: ~/Library/Application Support/LajcoinCore
    // Unix-like: ~/.dashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LajcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/LajcoinCore";
#else
    // Unix-like
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure Lajcoin Core copyright is not removed by accident
    if (copyright_devs.find("Lajcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Lajcoin Core developers";
    }
