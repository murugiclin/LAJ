 ...
 *  We have 4 times smaller block times in Lajcoin, so we need to push 4 times more invs per 1MB. */
...
struct Peer {
     ...
     * (Lajcoin)    Enables the flag that allows GetTxRelay() to return m_tx_relay */
...
private:
     ...
     * (Lajcoin)    Always initialized but selectively available through GetTxRelay()
...
bool PeerManagerImpl::AlreadyHave(const CInv& inv)
{
    ...
    /*
        Lajcoin Related Inventory Messages
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        // LAJ this code also handles non-TXs (Lajcoin specific messages)
