...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (llmq::quorumInstantSendManager->IsWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin : tx.vin)
        {
            ...
            if (ptxConflicting)
            {
                // Transaction conflicts with mempool and RBF doesn't exist in Lajcoin
...
static int64_t nTimeLajcoinSpecific = 0;
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!m_chain_helper->special_tx->ProcessSpecialTxsInBlock(block, pindex, view, fJustCheck, fScriptChecks, state, mnlist_updates_opt)) {
        return error("ConnectBlock(LAJ): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // LAJ : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (m_isman->RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = m_isman->GetConflictingLock(*tx)) {
                if (has_chainlock) {
                    LogPrint(BCLog::ALL, "ConnectBlock(LAJ): chain-locked transaction %s overrides islock %s\n",
                    ...
                    LogPrintf("ERROR: ConnectBlock(LAJ): transaction %s conflicts with transaction lock %s\n", tx->GetHash().ToString(), conflictLock->txid.ToString());
    ...
    // LAJ : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!m_chain_helper->special_tx->CheckCreditPoolDiffForBlock(block, pindex, blockSubsidy, state)) {
        return error("ConnectBlock(LAJ): CheckCreditPoolDiffForBlock for block %s failed with %s",
                     pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (!m_chain_helper->mn_payments->IsBlockValueValid(block, pindex->nHeight, blockSubsidy + feeReward, strError, check_superblock)) {
        ...
        LogPrintf("ERROR: ConnectBlock(LAJ): %s\n", strError);
    ...
    if (!m_chain_helper->mn_payments->IsBlockPayeeValid(*block.vtx[0], pindex->pprev, blockSubsidy, feeReward, check_superblock)) {
        // NOTE: Do not punish, the node might be missing governance data
        LogPrintf("ERROR: ConnectBlock(LAJ): couldn't find masternode or superblock payments\n");
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeLajcoinSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Lajcoin specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeLajcoinSpecific * MICRO, nTimeLajcoinSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    if (!m_chain_helper->special_tx->ProcessSpecialTxsInBlock(block, pindex, inputs, false /*fJustCheck*/, false /*fScriptChecks*/, state, mnlist_updates_opt)) {
        return error("RollforwardBlock(LAJ): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (fAddressIndex) {
        if (!m_blockman.m_block_tree_db->WriteAddressIndex(addressIndex)) {
            return error("RollforwardBlock(LAJ): Failed to write address index");
        }

        if (!m_blockman.m_block_tree_db->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            return error("RollforwardBlock(LAJ): Failed to write address unspent index");
        }
    }

    if (fSpentIndex) {
        if (!m_blockman.m_block_tree_db->UpdateSpentIndex(spentIndex))
            return error("RollforwardBlock(LAJ): Failed to write transaction index");
    }

    if (fTimestampIndex) {
        if (!m_blockman.m_block_tree_db->WriteTimestampIndex(CTimestampIndexKey(pindex->nTime, pindex->GetBlockHash())))
            return error("RollforwardBlock(LAJ): Failed to write timestamp index");
    }
...
bool CChainState::ReplayBlocks()
{
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        ...
        if (fDIP0003Active && !m_evoDb.VerifyBestBlock(pindexOld->GetBlockHash())) {
            return error("ReplayBlocks(LAJ): Found EvoDB inconsistency");
        }
    }
