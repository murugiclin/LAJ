...
enum ServiceFlags : uint64_t {
    ...
    // set by all Lajcoin Core non pruned nodes, and is unset by SPV clients or other light clients.
    NODE_NETWORK = (1 << 0),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // Lajcoin Core nodes used to support this by default, without advertising this bit,
...
enum GetDataMsg : uint32_t {
    ...
    MSG_SPORK = 6,
    /* 7 - 15 were used in old Lajcoin versions and were mainly budget and MN broadcast/ping related*/
    MSG_DSTX = 16,
