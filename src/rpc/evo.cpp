    ...
    static const std::map<std::string, RPCArg> mapParamHelp = {
        {"collateralAddress",
            {"collateralAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The Lajcoin address to send the collateral to."}
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            {"ownerAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The Lajcoin address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            {"payoutAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The Lajcoin address to use for masternode reward payments."}
        },
        ...
        {"platformP2PPort",
            {"platformP2PPort", RPCArg::Type::NUM, RPCArg::Optional::NO,
                "TCP port of Lajcoin Platform peer-to-peer communication between nodes (network byte order)."}
        },
        {"platformHTTPPort",
...
static RPCHelpMan protx_register_fund_wrapper(const bool legacy)
{
    ...
    return RPCHelpMan{rpc_full_name,
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 Lajcoin\n"
...
static RPCHelpMan protx_register_fund_evo()
{
    ...
    return RPCHelpMan{
        command_name,
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 4000 Lajcoin\n"
        "to the address specified by collateralAddress and will then function as the collateral of your\n"
        "EvoNode.\n"
        "A few of the limitations you see in the arguments are temporary and might be lifted after DIP3\n"
        "is fully deployed.\n" +
                                              ...
                                              const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Lajcoin address: ") + request.params[paramIdx + 6].get_str());
    }
...
static UniValue protx_update_service_common_wrapper(const JSONRPCRequest& request, const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 1].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Lajcoin address: ") + request.params[paramIdx + 1].get_str());
    } else {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Lajcoin address: ") + request.params[4].get_str());
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[3].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Lajcoin address: ") + request.params[3].get_str());
