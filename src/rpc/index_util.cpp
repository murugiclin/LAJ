...
{
    if (!fAddressIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Address index is disabled. You should run Lajcoin Core with -addressindex (requires reindex)");
    }
}
                   ...
                   CSpentIndexValue& value)
{
    ...
    if (!fSpentIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Spent index is disabled. You should run Lajcoin Core with -spentindex (requires reindex)");
    }
                       ...
                       std::vector<uint256>& hashes)
{
    ...
    if (!fTimestampIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Timestamp index is disabled. You should run Lajcoin Core with -timestampindex (requires reindex)");
    }
