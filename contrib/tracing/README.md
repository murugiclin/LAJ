...
Tracing (USDT) support for Lajcoin Core on Linux using. For more information on
USDT support in Lajcoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `dashd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `dashd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Lajcoin Core USDT documentation] on how to list available tracepoints in your
`dashd` binary.

[Lajcoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
Lajcoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
In a different terminal, starting Lajcoin Core in SigNet mode and with
re-indexing enabled.
