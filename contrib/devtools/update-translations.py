...
# Regexp to check for Lajcoin addresses
ADDRESS_REGEXP_LAJ = re.compile('[X7][a-zA-Z0-9]{30,}')
...
def contains_dash_addr(text, errors):
    if text is not None and ADDRESS_REGEXP_LAJ.search(text) is not None:
        errors.append('Translation "%s" contains a Lajcoin address. This will be removed.' % (text))
        return True
    return False
