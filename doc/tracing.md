# User-space, Statically Defined Tracing (USDT) for Lajcoin Core

Lajcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Lajcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Lajcoin Core
...
To list probes in Lajcoin Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Lajcoin Core.
