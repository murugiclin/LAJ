...
The data directory is the default location where the Lajcoin Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.dashcore/`
macOS    | `$HOME/Library/Application Support/Lajcoincore/`
Windows  | `%APPDATA%\Lajcoincore\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Lajcoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
These subdirectories and files are no longer used by Lajcoin Core:
