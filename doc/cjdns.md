# CJDNS support in Lajcoin Core

It is possible to run Lajcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Lajcoin Core, similarly to a VPN (either in the host/OS or on
the network router).
...
redundancy and robustness for both the Lajcoin network and individual nodes.
...
## Run Lajcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Lajcoin Core
...
When enabled, this option tells Lajcoin Core that it is running in an
...
IPv6 local network. This helps Lajcoin Core perform better address management:
...
CJDNS support was added to Lajcoin Core in version 21.0 and there may be fewer
...
## CJDNS-related information in Lajcoin Core

There are several ways to see your CJDNS address in Lajcoin Core:
