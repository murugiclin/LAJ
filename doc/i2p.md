# I2P support in Lajcoin Core

It is possible to run Lajcoin Core as an
...
## Run Lajcoin Core with an I2P router (proxy)

A running I2P router (proxy) is required with the [SAM](https://geti2p.net/en/docs/api/samv3)
application bridge enabled. The following routers are recommended for use with Lajcoin Core:
...
Once an I2P router with SAM enabled is up and running, use the following Lajcoin
...
I2P support was added to Lajcoin Core in version 20.0 and there may be fewer I2P
...
The first time Lajcoin Core connects to the I2P router, it automatically
...
`i2p_private_key` in the Lajcoin Core data directory.  The persistent I2P
...
## Fetching I2P-related information from Lajcoin Core

There are several ways to see your I2P address in Lajcoin Core if accepting
...
Lajcoin Core uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3) protocol
to connect to the I2P network. Any I2P router that supports it can be used.

## Ports in I2P and Lajcoin Core
...
To allow future upgrades to newer versions of SAM, Lajcoin Core sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of Lajcoin Core.
...
It's important that the nodes of a popular application like Lajcoin contribute
...
## Bundling I2P in a Lajcoin application

Please see the "General Guidance for Developers" section in https://geti2p.net/en/docs/api/samv3
if you are developing a downstream application that may be bundling I2P with Lajcoin.
