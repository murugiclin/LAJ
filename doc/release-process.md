...
### Lajcoin Core maintainers/release engineers, suggestion for writing release notes
...
Checkout the Lajcoin Core version you'd like to build:
...
* [ ] Release on Lajcoin forum: https://www.dash.org/forum/topic/official-announcements.54/ (necessary so we have a permalink to use on twitter, reddit, etc.)
...
* [ ] Test Docker build runs without error in Lajcoinmate
...
After successfully notarizing the .dmg file, extract `Lajcoin-Qt.app` from the .dmg.
To verify that the notarization process was successful, run the following command:

```sh
spctl -a -vv -t install Lajcoin-Qt.app
```

Replace `Lajcoin-Qt.app` with the path to your .app file. This command checks whether your .app file passes Gatekeeper’s
