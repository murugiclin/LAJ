UNIX BUILD NOTES
====================
Some notes on how to build Lajcoin Core in Unix.

(For BSD specific instructions, see `build-*bsd.md` in this directory.)

Note
---------------------
Always use absolute paths to configure and compile Lajcoin Core and the dependencies.
...
memory available when compiling Lajcoin Core. On systems with less, gcc can be
...
To build Lajcoin Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
Otherwise, you can build Lajcoin Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

Security
--------
To help make your Lajcoin Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, Lajcoin Core should be built with a non-executable stack,
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, Lajcoin Core may be compiled in
...
As mentioned above, when maintaining portability of the wallet between the standard Lajcoin Core distributions and independently built
