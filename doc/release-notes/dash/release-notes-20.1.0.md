...
In this release, we are taking a significant step towards enhancing the Lajcoin wallet's usability by enabling Hierarchical
...
While HD wallets are now enabled by default to improve user experience, Lajcoin Core still allows for the creation of
...
the v18.1 release.
- The `startupnotify` option is used to specify a command to execute when Lajcoin Core has finished with its startup
sequence.
...
- New Discreet mode for hiding balances in Lajcoin-Qt
- Added a "Close all wallet" menu item for Lajcoin-Qt
