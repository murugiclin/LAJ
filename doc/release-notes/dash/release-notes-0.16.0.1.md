...
first startup of Lajcoin Core will run a migration process which can take a few
...
to slow the growth rate of Lajcoin’s circulating supply by encouraging the
...
-----------------------------
In Lajcoin we have used lower thresholds (80% vs 95% in BTC) to activate upgrades
...
--------------------------------
This version of Lajcoin Core includes multiple optimizations to the network and p2p message
...
When Lajcoin Core is not started with any `--wallet=<path>` options, the name of
the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
now the empty string `""` instead of `"wallet.dat"`. If Lajcoin Core is started
...
`--debug-ui` will force Lajcoin Core to reload the custom css files as soon as they get updated
which makes it possible to see and debug all css adjustments live in the running GUI.

From now on the "Pay To" field in "Send" and "PrivateSend" tabs also accepts Lajcoin URIs.
The Lajcoin address and the amount from the URI are assigned to corresponding fields automatically
if a Lajcoin URI gets pasted into the field.
...
Build system
------------
The minimum version of the GCC compiler required to compile Lajcoin Core is now 4.8.
...
backported into Lajcoin Core 0.16. We only excluded backports which do not align
with Lajcoin, like SegWit or RBF related changes.
...
- [`da38e2bf83`](https://github.com/dashpay/dash/commit/da38e2bf83) Change litemode from disabling all Lajcoin specific features to disabling governance validation (#3488)
...
- [`b4008ee4fc`](https://github.com/dashpay/dash/commit/b4008ee4fc) Some Lajcoinification (#3513)
...
- [`fe208c98e3`](https://github.com/dashpay/dash/commit/fe208c98e3) Feat. request for Lajcoin Platform: `quorum sign` rpc command with additional quorumHash #3424 (#3446)
...
- [`75bb7ec022`](https://github.com/dashpay/dash/commit/75bb7ec022) A few optimizations/speedups for Lajcoin related tests (#3268)
