...
Currently, 10% of block rewards are set aside for the Lajcoin DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Lajcoin in a
credit pool. This credit pool mirrors the available credit supply in Lajcoin
Platform. When a certain amount of Lajcoin is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Lajcoin
Platform. Masternode identities receive the fees and can convert them back to
Lajcoin by withdrawing them from Lajcoin Platform.
...
This feature has been disabled by default since Lajcoin Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Lajcoin Core version 20.0.0 there are extra changes:
...
- It is possible to run Lajcoin Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `LAJ/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Lajcoin Core (BIP61)_](#removal-of-reject-network-messages-from-dash-core-bip61) above for
...
with Lajcoin’s product needs, such as SegWit and RBF, are excluded from our
