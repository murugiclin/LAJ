# TOR SUPPORT IN LAJ CORE

It is possible to run Lajcoin Core as a Tor onion service, and connect to such services.
...
- Starting with version 20.0, Lajcoin Core only supports Tor version 3 hidden
  services (Tor v3). Tor v2 addresses are ignored by Lajcoin Core and neither
  relayed nor stored.

- Tor removed v2 support beginning with version 0.4.6.

## How to see information about your Tor configuration via Lajcoin Core

There are several ways to see your local onion address in Lajcoin Core:
...
## 1. Run Lajcoin Core behind a Tor proxy

The first step is running Lajcoin Core behind a Tor proxy. This will already anonymize all
...
## 2. Automatically create a Lajcoin Core onion service

Lajcoin Core makes use of Tor's control socket API to create and destroy
ephemeral onion services programmatically. This means that if Tor is running and
proper authentication has been configured, Lajcoin Core automatically creates an
onion service to listen on. The goal is to increase the number of available
onion nodes.

This feature is enabled by default if Lajcoin Core is listening (`-listen`) and
...
## 3. Manually create a Lajcoin Core onion service
...
should be equal to binding address and port for inbound Tor connections (127.0.0.1:9996 by default).
    ...
    -externalip=X   You can tell Lajcoin Core about its publicly reachable addresses using
...
## 3.1. List of known Lajcoin Core Tor relays
...
- Do not add anything but Lajcoin Core ports to the onion service created in section 3.
